...
// POST /products
router.post("/", (req, res) => {
    const { title, price,condition,category,description, seller,location,isSold,createdAt  } = req.body;

    if (!title || !price || !condition || !category || !description || !seller || !location || !isSold || !createdAt) {
        return res.status(400).json({ error: "title, price,condition,category,description, seller,location,isSold,createdAt are required Fields" });
    }

    const newProduct = { id: nextId++, title, price,condition,category,description, seller,location,isSold,createdAt };
...
router.delete("/:id", (req, res) => {
    ...
    res.json({ message: "Products deleted", deleted });
});
